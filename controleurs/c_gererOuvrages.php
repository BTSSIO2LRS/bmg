...
switch ($action) {
    ...
    case 'ajouterOuvrage' : {
        ...
        $chargeLesAuteurs = Auteurs::listeAuteurEx(array());
        $lesAuteurs = array();
        foreach($chargeLesAuteurs as $values)
        {
            $lesAuteurs[$values["id_auteur"]] = $values["nom_auteur"];
        }
    ...
    // case gérant les auteurs d'un ouvrage
    case 'auteursOuvrage':
    {
    ...
    case'ajouterAuteur':
    {
        ...
        // On charge les auteurs pour les lister afin de pouvoir sélectionner l'auteur de l'ouvrage
                $chargeLesAuteurs = Auteurs::listeAuteurEx($unOuvrage->getAuteurs());
                $lesAuteurs = array();
                foreach($chargeLesAuteurs as $values)
                {
                    $lesAuteurs[$values['id_auteur']] = $values['nom_auteur'];
